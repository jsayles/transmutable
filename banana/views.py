...
def index(request):
	return render_to_response('banana/index.html', { 'completed_items':CompletedItem.objects.recent(max_count=10, created_after=datetime.datetime.now() - datetime.timedelta(days=4), viewer_id=request.user.id), 'users':User.objects.all().order_by('-work_docs__modified') }, context_instance=RequestContext(request))
